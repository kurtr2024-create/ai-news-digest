  ...
  return (
    <aside className="flex flex-col gap-4">
      <div className="rounded-xl border border-[#1f1f1f] bg-[#111111] p-5">
        <div className="mb-1 flex items-center gap-2"><span className="text-xl">🔥</span><span className="text-base font-bold text-[#f9fafb]">{state.streak} day{state.streak !== 1 ? "s" : ""} streak</span></div>
        ...
        <div className="flex items-center gap-1.5">{last7.map(({ date, visited }) => <div key={date} title={date} className={`h-5 w-5 rounded-full transition-colors ${visited ? "bg-blue-500" : "bg-[#1f1f1f]"}`} />)}</div>
      </div>
      <div className="rounded-xl border border-[#1f1f1f] bg-[#111111] p-5">
        <div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-semibold text-[#f9fafb]">Today&apos;s Goal</h3><span className="text-xs tabular-nums text-[#6b7280]">{goal.complete}/3</span></div>
        <div className="mb-4 h-1 w-full overflow-hidden rounded-full bg-[#1f1f1f]"><div className="h-full rounded-full bg-blue-500 transition-all duration-500" style={{ width: `${(goal.complete / 3) * 100}%` }} /></div>
        <div className="flex flex-col gap-2.5">{goalItems.map((item, i) => (
      ...
      <div className="rounded-xl border border-[#1f1f1f] bg-[#111111] p-5">
        ...
        <div className="grid grid-cols-2 gap-2">{BADGES.map(badge => {
