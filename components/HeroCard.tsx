...
interface TopArticle { title: string; source: string; url: string; summary: string; publishedAt: string; imageDescription?: string; }

export default function HeroCard({ topArticle }: { topArticle: TopArticle }) {
  const handleShare = () => {
    const text = `${topArticle.title} - via AI News Daily Digest`;
    const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(topArticle.url)}`;
  ...
  return (
    <div className="rounded-xl border border-[#1f1f1f] bg-[#111111] p-8" style={{ borderLeft: "2px solid #3b82f6" }}>
      ...
      <div className="flex items-center gap-4">
        <a href={topArticle.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm font-medium text-blue-500 transition-colors hover:text-blue-400">
