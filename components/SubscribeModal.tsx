...
export default function SubscribeModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        if (raw) { const st = JSON.parse(raw); st.email = email.trim(); localStorage.setItem("ai-digest-gamification", JSON.stringify(st)); }
      } catch {}
      setStatus("success");
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="relative w-full max-w-md rounded-2xl border border-[#1f1f1f] bg-[#111111] p-8 shadow-2xl">
        ...
        {status === "success" ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required autoFocus
              className="rounded-xl border border-[#1f1f1f] bg-[#0a0a0a] px-4 py-3 text-sm text-[#f9fafb] placeholder-[#4b5563] outline-none transition-colors focus:border-blue-500/50" />
            {status === "error" && <p className="text-xs text-red-400">{errorMsg}</p>}
            <button type="submit" disabled={status === "loading"} className="rounded-xl bg-blue-500 py-3 text-sm font-medium text-white transition-colors hover:bg-blue-600 disabled:opacity-50">
