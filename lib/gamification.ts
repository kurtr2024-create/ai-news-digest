...
function yesterdayISO(): string { const d = new Date(); d.setDate(d.getDate() - 1); return d.toISOString().split("T")[0]; }

export function loadState(): GamificationState {
  if (typeof window === "undefined") return { ...DEFAULT_STATE };
  try { const raw = localStorage.getItem(STORAGE_KEY); if (!raw) return { ...DEFAULT_STATE }; return { ...DEFAULT_STATE, ...JSON.parse(raw) }; } catch { return { ...DEFAULT_STATE }; }
}
...
export function recordVisit(state: GamificationState): GamificationState {
  ...
  if (state.lastVisitDate === today) {} else if (state.lastVisitDate === yesterday) { newStreak = state.streak + 1; } else { newStreak = 1; }
  const history = state.visitHistory.filter(d => d !== today); history.push(today);
